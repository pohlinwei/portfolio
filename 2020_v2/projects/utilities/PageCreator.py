...
class PageCreator:
    ...
    DESCRIPTION_TEMPLATE = Path('templates/description_template.html')
    ...
    def generate_content(self, project):
        ...
        description = self.get_description_html(project.get_description())
        ...
        return main_template.format(NAME=name, DESCRIPTION=description, GITHUB_BUTTON=github_button, IMAGES=images)
    ...
    def get_description_html(self, description):
        description_template = PageCreator.DESCRIPTION_TEMPLATE.read_text()
        return description_template.format(DESCRIPTION=description)
